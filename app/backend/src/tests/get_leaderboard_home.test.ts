...
import { homeTeamRankings } from './data/leaderboard';
chai.use(chaiHttp);
...
describe('Testando o endpoint GET /leaderboard/home', () => {
  ...
  it('deve retornar uma lista com as classificações dos times da casa', async () => {
    expect(chaiHttpResponse.body).to.deep.equal(homeTeamRankings);
  });
});
